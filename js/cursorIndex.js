	...
	const appbg = new PIXI.Application({ view: canvasCursor, transparent: true, antialias: true, resizeTo: window});
	...
	appbg.ticker.add(() =>	{
		...
		if (mouseStatusCTA == true){
			gsap.to(cursor,0.5,{pixi:{x: window.innerWidth/2, y: window.innerHeight/2, scale: 4,}});
			gsap.to(cursorFill,0.5,{pixi:{x: window.innerWidth/2, y: window.innerHeight/2, scale: 4 }});
			...
			gsap.to(cursor,0.25,{pixi:{x: mousePositionBg.x, y: mousePositionBg.y, scale: 1	}});
			gsap.to(cursorFill,0.25,{pixi:{x: mousePositionBg.x, y: mousePositionBg.y, scale: 0}});
